...
java -jar activiti-liquibase.jar --url=jdbc:postgresql://${dbServerName}:${dbPort}/${contextName}activiti?sslmode=require --username=${dbUserName} --password=${dbPassword} --logLevel=info update
if [ $? -ne 0 ]
then
    exit 1
else
