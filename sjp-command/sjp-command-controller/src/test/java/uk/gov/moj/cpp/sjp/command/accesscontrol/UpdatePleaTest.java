...
public class UpdatePleaTest extends BaseDroolsAccessControlTest {
    ...
    public void shouldAllowUpdatePleaCommand() {
        final JsonObject inputPayload = createObjectBuilder()
                ...
                .build();
        final JsonEnvelope envelope = JsonEnvelopeBuilder.envelopeFrom(metadataWithRandomUUID("sjp.command.update-plea").build(), inputPayload);
    ...
    public void shouldAllowCancellPleaCommand() {
        final JsonObject inputPayload = createObjectBuilder()
                .add("caseId", randomUUID().toString())
                .build();
        final JsonEnvelope envelope = JsonEnvelopeBuilder.envelopeFrom(metadataWithRandomUUID("sjp.command.cancel-plea").build(), inputPayload);
        final Action action = new Action(envelope);
        assertSuccessfulOutcome(executeRulesWith(action));
    }
}