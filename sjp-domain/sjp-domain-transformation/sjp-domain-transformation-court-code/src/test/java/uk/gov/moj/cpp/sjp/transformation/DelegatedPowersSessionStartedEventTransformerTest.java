...
public class DelegatedPowersSessionStartedEventTransformerTest {

    private static final String EVENT_DELEGATED_POWERS_SESSION_STARTED = "sjp.events.delegated-powers-session-started";
    ...
    @InjectMocks
    private DelegatedPowersSessionStartedEventTransformer delegatedPowersSessionStartedEventTransformer;
    ...
    @Before
    public void setUp() {
        delegatedPowersSessionStartedEventTransformer.setEnveloper(createEnveloper());
    }
    ...
    public void shouldIgnoreIrrelevantEvents() {
        ...
        final Action action = delegatedPowersSessionStartedEventTransformer.actionFor(envelope);
        assertThat(action, CoreMatchers.is(NO_ACTION));
    }

    @Test
    public void shouldTransformDelegatedPowersSessionStartedEvent() {
        final JsonEnvelope envelope = envelopeFrom(metadataWithRandomUUID(EVENT_DELEGATED_POWERS_SESSION_STARTED), JsonValue.NULL);
    ...
    public void shouldEnrichEventWithCourtHouseCode() {
        ...
        final Stream<JsonEnvelope> jsonEnvelopeStream = delegatedPowersSessionStartedEventTransformer.apply(buildEnvelope());
        ...
        assertThat(events.get(0), jsonEnvelope(
                metadata().withName(EVENT_DELEGATED_POWERS_SESSION_STARTED),
                payloadIsJson(allOf(
    ...
    public void shouldIgnoreEventWithCourtCodePresent() {
        ...
        final JsonEnvelope envelope = envelopeFrom(metadataWithRandomUUID(EVENT_DELEGATED_POWERS_SESSION_STARTED), payload);

        final Stream<JsonEnvelope> jsonEnvelopeStream = delegatedPowersSessionStartedEventTransformer.apply(envelope);
    ...
    private JsonEnvelope buildEnvelope() {
        ...
        return envelopeFrom(metadataWithRandomUUID(EVENT_DELEGATED_POWERS_SESSION_STARTED), payload);
    }
