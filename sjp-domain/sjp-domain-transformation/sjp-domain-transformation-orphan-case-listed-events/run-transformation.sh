...
TRANSFORMATION_JAR=`echo target/sjp-domain-transformation-orphan-case-listed-events-*.jar`
...
[[ ! -f ${EVENT_TOOL_JAR} ]] && \
    curl https://libraries-internal.mdv.cpp.nonlive/artifactory/repocentral/uk/gov/justice/event-tool/${EVENT_TOOL_VERSION}/event-tool-${EVENT_TOOL_VERSION}-swarm.jar > ${EVENT_TOOL_JAR}

touch ${PROCESS_FILE}

java -jar  -Dorg.wildfly.swarm.mainProcessFile=${PROCESS_FILE} \
    -DstreamCountReportingInterval=3 -Devent.transformation.jar=${TRANSFORMATION_JAR} ${EVENT_TOOL_JAR} \
