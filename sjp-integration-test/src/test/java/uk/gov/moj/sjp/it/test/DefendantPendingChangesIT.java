...
import static uk.gov.moj.sjp.it.Constants.DEFENDANT_PENDING_CHANGES_ACCEPTED_EVENT;
import static uk.gov.moj.sjp.it.Constants.DEFENDANT_PENDING_CHANGES_ACCEPTED_PUBLIC_EVENT;
import static uk.gov.moj.sjp.it.Constants.DEFENDANT_PENDING_CHANGES_REJECTED_EVENT;
...
public class DefendantPendingChangesIT extends BaseIntegrationTest {
    ...
    public void shouldAcceptDefendantChangesAndRaisePublicEvent() {
        ...
        final EventListener defendantPendingChangesAcceptedListener = new EventListener()
                .subscribe(DEFENDANT_PENDING_CHANGES_ACCEPTED_EVENT, DEFENDANT_PENDING_CHANGES_ACCEPTED_PUBLIC_EVENT)
                .run(() -> UpdateDefendantDetails.acceptDefendantPendingChangesForCaseAndPayload(caseIdOne, defendantId, payloadBuilder));

        final Optional<JsonEnvelope> defendantPendingChangesAcceptedEvent = defendantPendingChangesAcceptedListener.popEvent(DEFENDANT_PENDING_CHANGES_ACCEPTED_EVENT);
        ...
        assertThat(defendantPendingChangesAcceptedEvent.isPresent(), CoreMatchers.is(true));
        assertThat(defendantPendingChangesAcceptedPublicEvent.isPresent(), CoreMatchers.is(true));
        assertThat(defendantPendingChangesAcceptedEvent.get().payloadAsJsonObject().getString("caseId"), CoreMatchers.is(caseIdOne.toString()));
    ...
    public void shouldRejectDefendantChangesAndRaisePublicEvent() {
        ...
        final EventListener defendantPendingChangesRejectedListener = new EventListener()
                .subscribe(DEFENDANT_PENDING_CHANGES_REJECTED_EVENT, DEFENDANT_PENDING_CHANGES_REJECTED_PUBLIC_EVENT)
                .run(() -> UpdateDefendantDetails.rejectDefendantPendingChanges(caseIdOne, defendantId));

        final Optional<JsonEnvelope> defendantPendingChangesRejectedEvent = defendantPendingChangesRejectedListener.popEvent(DEFENDANT_PENDING_CHANGES_REJECTED_EVENT);
        ...
        assertThat(defendantPendingChangesRejectedEvent.isPresent(), CoreMatchers.is(true));
        assertThat(defendantPendingChangesRejectedPublicEvent.isPresent(), CoreMatchers.is(true));
        assertThat(defendantPendingChangesRejectedEvent.get().payloadAsJsonObject().getString("caseId"), CoreMatchers.is(caseIdOne.toString()));
