...
public class PressTransparencyReportIT extends BaseIntegrationTest {
    ...
    public void shouldGeneratePressTransparencyReports() throws IOException {
        ...
        final UUID generatedDocumentId = createStubFile("press-transparency-report.pdf", ZonedDateTime.now());
        ...
        final EventListener metadataAddedEventListener = new EventListener()
                ...
                .run(() -> publishSysDocPublicEvent(pressTransparencyReportId, generatedDocumentId));
        ...
        final Matcher matcher = withJsonPath("fileId", equalTo(generatedDocumentId.toString()));

        final JsonObject apiReportMetadata = pressTransparencyReportHelper.pollForPressTransparencyReportMetadata(matcher);
        validateMetadata(apiReportMetadata, generatedDocumentId.toString());
    ...
    private JsonObject documentAvailablePayload(final UUID templatePayloadId, final String reportId, final UUID generatedDocumentId) {
    ...
    private void validateMetadata(final JsonObject apiReportMetadata, final String generatedFileId) {
        assertThat(apiReportMetadata.getString("fileId"), is(generatedFileId));
        assertThat(apiReportMetadata.get("size").toString(), is(notNullValue()));
        assertThat(apiReportMetadata.getInt("pages"), is(2));
        assertThat(apiReportMetadata.getString("generatedAt"), is(notNullValue()));
    }
    ...
    private void validateDocumentGenerationRequest(final JSONObject docGenerationRequest,
        ...
        assertThat(docGenerationRequest.getString("templateIdentifier"), is(TEMPLATE_NAME));
