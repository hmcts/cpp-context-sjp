...
public class OffenceDecorator {
    ...
    public JsonObject decorateAllOffences(final JsonObject caseView,  final JsonEnvelope envelope, final WithdrawalReasons withdrawalReasons) {
        return createObjectBuilder(caseView)
                .add("defendant", decorateDefendantOffences(caseView.getJsonObject("defendant"), caseView.getJsonArray("caseDecisions"), envelope, withdrawalReasons))
                .build();
    }

    private JsonObjectBuilder decorateDefendantOffences(final JsonObject defendantView, final JsonArray caseDecisions, final JsonEnvelope envelope, final WithdrawalReasons withdrawalReasons) {
        return createObjectBuilder(defendantView)
                .add("offences", defendantView.getJsonArray("offences")
                        .getValuesAs(JsonObject.class)
                        .stream()
                        .map(offence -> decorateOffence(offence, caseDecisions, envelope, withdrawalReasons))
                        .reduce(createArrayBuilder(), JsonArrayBuilder::add, JsonArrayBuilder::add));
    }

    private JsonObjectBuilder decorateOffence(final JsonObject offenceInstance, final JsonArray caseDecisions, final JsonEnvelope envelope, final WithdrawalReasons withdrawalReasons) {
