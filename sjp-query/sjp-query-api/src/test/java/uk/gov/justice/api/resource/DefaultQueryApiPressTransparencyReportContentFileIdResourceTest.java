...
public class DefaultQueryApiPressTransparencyReportContentFileIdResourceTest {
    ...
    @InjectMocks
    private DefaultQueryApiPressTransparencyReportContentFileIdResource underTest;
    ...
    @Test
    public void shouldReturnValidResponseWhenFileIdIsValid() {
        when(blobContainerClient.getBlobClient(internal().blobName(fileId))).thenReturn(blobClient);
        when(blobClient.exists()).thenReturn(true);

        final Response response = underTest.getPressTransparencyReportContentByFileId(fileId, userId);
        ...
        assertThat(response.getHeaderString(CONTENT_DISPOSITION), is("attachment;filename=PressTransparencyReport_" + fileId + ".pdf"));
    ...
    @Test
    public void shouldThrowExceptionWhenFileIdIsNotValid() {
        when(blobContainerClient.getBlobClient(internal().blobName(fileId))).thenReturn(blobClient);
        when(blobClient.exists()).thenReturn(false);

        assertThrows(RuntimeException.class, () -> underTest.getPressTransparencyReportContentByFileId(fileId, userId));
    }
}
