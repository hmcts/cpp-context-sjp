...
import static uk.gov.moj.cpp.sjp.query.api.RuleConstants.getQueryCasesSearchActionGroups;
...
public class CaseSearchTest extends BaseDroolsAccessControlTest {
    ...
    public void shouldAllowUserInAuthorisedGroupToSearchCases() {
        given(userAndGroupProvider.isMemberOfAnyOfTheSuppliedGroups(action, getQueryCasesSearchActionGroups())).willReturn(true);
        assertSuccessfulOutcome(executeRulesWith(action));
    }

    @Test
    public void shouldNotAllowUserNotInAuthorisedGroupToSearchCases() {
        given(userAndGroupProvider.isMemberOfAnyOfTheSuppliedGroups(action, getQueryCasesSearchActionGroups())).willReturn(false);
    ...
    public void tearDown() {
        ...
        assertThat(listCaptor.getValue(), containsInAnyOrder(getQueryCasesSearchActionGroups().toArray()));
