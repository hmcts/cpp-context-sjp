...
public class QueryCaseByUrnTest extends BaseDroolsAccessControlTest {
    ...
    private static final String CONTENT_TYPE = "sjp.query.case-by-urn";
    ...
    public void shouldAllowUserInAuthorisedGroupToQueryCaseByUrn() {
        given(userAndGroupProvider.isMemberOfAnyOfTheSuppliedGroups(action, getQueryCaseByUrnActionGroups())).willReturn(true);
        assertSuccessfulOutcome(executeRulesWith(action));
    }

    @Test
    public void shouldNotAllowUserNotInAuthorisedGroupToQueryCaseByUrn() {
        given(userAndGroupProvider.isMemberOfAnyOfTheSuppliedGroups(action, getQueryCaseActionGroups())).willReturn(false);
    ...
    public void tearDown() {
        ...
        assertThat(listCaptor.getValue(), containsInAnyOrder(getQueryCaseByUrnActionGroups().toArray()));
