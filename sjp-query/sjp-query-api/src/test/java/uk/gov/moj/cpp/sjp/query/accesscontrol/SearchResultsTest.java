...
public class SearchResultsTest extends BaseDroolsAccessControlTest {
    ...
    public void shouldAllow() {
        given(userAndGroupProvider.isMemberOfAnyOfTheSuppliedGroups(action, getQueryFindCaseSearchResultsActionGroups())).willReturn(true);
        assertSuccessfulOutcome(executeRulesWith(action));
    }

    @Test
    public void shouldNotAllow() {
        given(userAndGroupProvider.isMemberOfAnyOfTheSuppliedGroups(action, getQueryFindCaseSearchResultsActionGroups())).willReturn(false);
    ...
    public void tearDown() {
        ...
        assertThat(listCaptor.getValue(), containsInAnyOrder(getQueryFindCaseSearchResultsActionGroups().toArray()));
